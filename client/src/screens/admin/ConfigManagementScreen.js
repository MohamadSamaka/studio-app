...
const ConfigManagementScreen = () => {
  ...
  useEffect(() => {
    if (config) {
      setLocalConfig({
        cancelationRefundThresholdTime:
          config.reservations["cancelation-refund-threshold-time"] || "00:00",
        maxUsersPerReservation:
          config.reservations["max-users-per-reservation"]?.toString() || "",
      ...
      const newConfig = {
        reservations: {
          "cancelation-refund-threshold-time":
            localConfig.cancelationRefundThresholdTime || "00:00",
          "max-users-per-reservation":
            parseInt(localConfig.maxUsersPerReservation, 10) || 0,
  ...
  const formatTime = (time) => {
    ...
    const validHours =
      !isNaN(hours) && hours >= 0 && hours <= 24 ? padZero(hours) : "00";
    const validMinutes =
      !isNaN(minutes) && minutes >= 0 && minutes < 60 ? padZero(minutes) : "00";
  ...
  const validateInputs = () => {
    ...
    if (!cancelationRefundThresholdTime?.match(/^\d{2}:\d{2}$/)) {
      showSnackbar(
        "Invalid format for Cancelation Refund Threshold Time. Use HH:MM.",
        "error"
      );
      return false;
    }
    ...
    if (isNaN(maxUsers) || maxUsers <= 0 || !Number.isInteger(maxUsers)) {
      showSnackbar(
        "Max Users Per Reservation must be a positive whole number.",
        "error"
      );
      return false;
    }
    ...
    if (!timeslotsDuration?.match(/^\d{2}:\d{2}$/)) {
      showSnackbar(
        "Invalid format for Timeslots Duration. Use HH:MM.",
        "error"
      );
      return false;
    }

    const [refundHours, refundMinutes] = cancelationRefundThresholdTime
      .split(":")
      .map(Number);
    if (refundHours > 24 || (refundHours === 24 && refundMinutes !== 0)) {
      showSnackbar(
        "Cancelation Refund Threshold Time cannot exceed 24:00.",
        "error"
      );
      return false;
    }
            ...
            <View style={{ minHeight: 600 }}>
              ...
              >
                <Button
                  mode="outlined"
                  onPress={() =>
                    openTimePicker("cancelationRefundThresholdTime")
                  }
                  style={styles.button}
                ...
                <TimePickerModal
                  ...
                  onConfirm={(time) =>
                    onConfirmTime("cancelationRefundThresholdTime", time)
                  }
                  hours={
                    parseInt(
                      localConfig.cancelationRefundThresholdTime?.split(":")[0],
                      10
                    ) || 0
                  }
                  minutes={
                    parseInt(
                      localConfig.cancelationRefundThresholdTime?.split(":")[1],
                      10
                    ) || 0
                  }
              ...
              >
                <TextInput
                  ...
                  onChangeText={(text) => {
                    ...
                    setLocalConfig({
                      ...localConfig,
                      maxUsersPerReservation: cleanedText,
                    });
                  }}
              ...
              >
                ...
                <TimePickerModal
                  ...
                  hours={
                    parseInt(
                      localConfig.timeslotsDuration?.split(":")[0],
                      10
                    ) || 0
                  }
                  minutes={
                    parseInt(
                      localConfig.timeslotsDuration?.split(":")[1],
                      10
                    ) || 0
                  }
